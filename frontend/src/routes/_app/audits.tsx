              ...
              <Table>
                ...
                <TableBody>
                  {audits.map((a) => {
                    ...
                    const disc = a.findings.filter(
                      (f) => f.status === "missing" || f.status === "damaged",
                    ).length;
                    ...
                    return (
                      <TableRow key={a.id}>
                        ...
                        <TableCell>
                          <span
                            className={
                              disc > 0 ? "text-destructive font-medium" : "text-muted-foreground"
                            }
                          >
                            {disc}
                          </span>
...
function CreateAuditDialog({ actorId, onClose }: { actorId: string; onClose: () => void }) {
  ...
  const employees = useStore(() =>
    store.employees.filter((e) => e.role === "admin" || e.role === "asset_manager"),
  );
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(v) => setForm((f) => ({ ...f, scopeDepartmentId: v }))}
          >
            <SelectTrigger>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-muted-foreground">
            {format(new Date(audit.startDate), "MMM d")} —{" "}
        ...
        {!locked && canClose && (
          <AlertDialog open={confirmClose} onOpenChange={setConfirmClose}>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Close audit?</AlertDialogTitle>
                <AlertDialogDescription>
                  Missing assets will be marked as Lost and the audit will be locked from further
                  editing.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <Card className="border-warning/40 bg-warning/5">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">
              Discrepancy Report — {disc.length} item{disc.length > 1 ? "s" : ""}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {disc.map((f) => {
                ...
                return (
                  <div key={f.id} className="flex items-center gap-2">
                    <StatusBadge status={f.status} /> <span className="font-medium">{a?.name}</span>{" "}
