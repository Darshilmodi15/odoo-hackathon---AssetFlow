                      ...
                      return (
                        <TableRow key={t.id}>
                          ...
                          <TableCell className="text-xs text-muted-foreground">
                            {format(new Date(t.requestedAt), "MMM d")}
                          </TableCell>
                          <TableCell>
                              ...
                              hasRole(["admin", "asset_manager", "department_head"]) && (
                                <div className="flex justify-end gap-1">
                                  <ConfirmButton
                                    label="Approve"
                                    onConfirm={async () => {
                                      await transferService.setStatus(t.id, "approved", user!.id);
                                  ...
                                  <ConfirmButton
                                    ...
                                    variant="ghost"
                                    onConfirm={async () => {
                                      await transferService.setStatus(t.id, "rejected", user!.id);
...
function AllocationList({
  ...
  allocations: typeof store.allocations;
  actorId: string;
  ...
  if (allocations.length === 0)
    return (
      <Card>
        <CardContent>
          <EmptyState title="No allocations to show" />
        </CardContent>
      </Card>
    );
            ...
            <TableBody>
              {allocations.map((a) => {
                ...
                return (
                  <TableRow
                    key={a.id}
                    className={highlightOverdue || overdue ? "bg-destructive/5" : ""}
                  >
                    <TableCell>
                      <div className="font-medium">{asset?.name}</div>
                      <div className="font-mono text-xs text-muted-foreground">{asset?.tag}</div>
                    </TableCell>
                    ...
                    <TableCell className="text-xs">
                      {a.expectedReturnAt ? (
                        <span className={overdue ? "text-destructive font-medium" : ""}>
                          {format(new Date(a.expectedReturnAt), "MMM d, yyyy")}
                        </span>
                      ) : (
                        "—"
                      )}
                    ...
                    <TableCell className="text-right">
                      {a.status !== "returned" && (
                        <Button size="sm" variant="outline" onClick={() => setReturning(a.id)}>
                          Return
                        </Button>
                      )}
                    </TableCell>
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Confirm the return and record the asset's condition.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={async () => {
              await allocationService.return(
                allocationId,
                { returnCondition: condition, returnNotes: notes },
                actorId,
              );
...
function AllocateDialog({ actorId, onClose }: { actorId: string; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      await allocationService.create(
        { assetId, employeeId, expectedReturnAt: expectedReturn || undefined, notes },
        actorId,
      );
  ...
  const requestTransfer = async () => {
    ...
    await transferService.create(
      { assetId, toEmployeeId: employeeId, reason: transferReason },
      actorId,
    );
              ...
              <SelectContent>
                {assets.map((a) => (
                  <SelectItem key={a.id} value={a.id}>
                    {a.tag} — {a.name} ({a.status.replace("_", " ")})
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      {conflict && !showTransfer && (
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {conflict.currentHolder && (
              <Button onClick={() => setShowTransfer(true)}>Create Transfer Request</Button>
            )}
          </div>
        </div>
      )}
      {showTransfer && (
        <div className="space-y-3">
          <div className="rounded-md bg-muted/50 p-3 text-sm">
            Requesting transfer of <b>{conflict?.asset.name}</b> ({conflict?.asset.tag}) from{" "}
            {conflict?.currentHolder?.name} to {employees.find((e) => e.id === employeeId)?.name}.
          </div>
          <div className="space-y-2">
            <Label>Reason</Label>
            <Textarea
